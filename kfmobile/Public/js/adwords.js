function Adwords(cc1){
	...
	this.conversionsContrast = function(arr){
		...
		if(arr.length){
			...
			$.ajax({
				type: "POST",
				url: "/Campaign/conversionsContrast",
				data: {"ids":arr,"date":$('#date').val()},
				dataType: "json",
				complete: function(){
					_this.showLoad(false);
	...
	this.iptcustomer = function(m){
		if(m == 'get'){
			...
			if(lock==0){
				...
				var urls = "/Home/Manager/iptcustomer";
